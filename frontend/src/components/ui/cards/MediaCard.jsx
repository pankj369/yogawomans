...
export default function MediaCard({ item, onClick, progress }) {
  return (
    ...
    >
      <div className="relative h-44 w-full overflow-hidden rounded-3xl border border-wellness-border bg-wellness-glass shadow-glass backdrop-blur-[18px] transition-all duration-300 group-hover:border-wellness-glow/30">
        <img
          src={item.image}
          alt={item.title}
          loading="lazy"
        ...
        <div className="absolute bottom-4 left-4 right-4 text-white">
          <p className="font-heading text-lg font-bold leading-tight line-clamp-1 group-hover:text-wellness-glow transition-colors">{item.title}</p>
